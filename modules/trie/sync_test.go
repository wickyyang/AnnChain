...
import (
	...
	"github.com/dappledger/AnnChain/modules/db"
)

// makeTestTrie create a sample test trie to test node-wise reconstruction.
func makeTestTrie() (db.Database, *Trie, map[string][]byte) {
	// Create an empty trie
	db, _ := db.NewMemDatabase()
	trie, _ := New(ethcmn.Hash{}, db)

	// Fill it with some arbitrary data
	content := make(map[string][]byte)
	for i := byte(0); i < 255; i++ {
		// Map the same data under multiple keys
		key, val := ethcmn.LeftPadBytes([]byte{1, i}, 32), []byte{i}
		content[string(key)] = val
		trie.Update(key, val)

		key, val = ethcmn.LeftPadBytes([]byte{2, i}, 32), []byte{i}
		content[string(key)] = val
		trie.Update(key, val)

		// Add some other data to inflate th trie
		for j := byte(3); j < 13; j++ {
			key, val = ethcmn.LeftPadBytes([]byte{j, i}, 32), []byte{j, i}
			content[string(key)] = val
			trie.Update(key, val)
		}
	}
...
func checkTrieContents(t *testing.T, db Database, root []byte, content map[string][]byte) {
	...
	trie, err := New(ethcmn.BytesToHash(root), db)
	if err != nil {
		t.Fatalf("failed to create trie at %x: %v", root, err)
	}
	if err := checkTrieConsistency(db, ethcmn.BytesToHash(root)); err != nil {
...
func checkTrieConsistency(db Database, root ethcmn.Hash) error {
...
func TestEmptyTrieSync(t *testing.T) {
	emptyA, _ := New(ethcmn.Hash{}, nil)
	emptyB, _ := New(emptyRoot, nil)

	for i, trie := range []*Trie{emptyA, emptyB} {
		db, _ := db.NewMemDatabase()
		if req := NewTrieSync(ethcmn.BytesToHash(trie.Root()), db, nil).Missing(1); len(req) != 0 {
...
func testIterativeTrieSync(t *testing.T, batch int) {
	...
	dstDb, _ := db.NewMemDatabase()
	sched := NewTrieSync(ethcmn.BytesToHash(srcTrie.Root()), dstDb, nil)

	queue := append([]ethcmn.Hash{}, sched.Missing(batch)...)
	for len(queue) > 0 {
...
func TestIterativeDelayedTrieSync(t *testing.T) {
	...
	queue := append([]ethcmn.Hash{}, sched.Missing(10000)...)
	for len(queue) > 0 {
...
func testIterativeRandomTrieSync(t *testing.T, batch int) {
	...
	queue := make(map[ethcmn.Hash]struct{})
...
func TestDuplicateAvoidanceTrieSync(t *testing.T) {
	...
	queue := append([]ethcmn.Hash{}, sched.Missing(0)...)
	requested := make(map[ethcmn.Hash]struct{})
...
func TestIncompleteTrieSync(t *testing.T) {
	...
	added := []ethcmn.Hash{}
	queue := append([]ethcmn.Hash{}, sched.Missing(1)...)
	for len(queue) > 0 {
