...
import ethcmn "github.com/dappledger/AnnChain/modules/common"
...
type nodeIteratorState struct {
	hash   ethcmn.Hash // Hash of the node being iterated (nil if not standalone)
	node   node        // Trie node being iterated
	parent ethcmn.Hash // Hash of the first full ancestor node (nil if current is the root)
...
type NodeIterator struct {
	...
	Hash     ethcmn.Hash // Hash of the current node being iterated (nil if not standalone)
			...
			for parent.child++; parent.child < len(node.Children); parent.child++ {
				if current := node.Children[parent.child]; current != nil {
					it.stack = append(it.stack, &nodeIteratorState{
						hash:   ethcmn.BytesToHash(node.flags.hash),
						node:   current,
						parent: ancestor,
						child:  -1,
					})
...
func (it *NodeIterator) retrieve() bool {
	...
	it.Hash, it.Node, it.Parent, it.Leaf, it.LeafBlob = ethcmn.Hash{}, nil, ethcmn.Hash{}, false, nil
